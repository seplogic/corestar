...
(* TODO(rgrig): Use the mechanism from [Debug]. *)
...
module type PCC =
    sig
      ...
      into [cc2]. It returns a possibly updated substitution, and the logical
      conjunction of [cc1] and [cc2].  PRE: [cc2] is supposed to already know
      about all the constants in the image of [subst]. *)
      val merge_cc
      ...
      let subst =
        ...
        fun c1 -> begin
          ...
          with Not_found -> begin
            ...
            with Not_found -> begin
              if log log_cc then
                fprintf logf "@[INTERNAL: 3inwf3: %a@\n@]" (pp_c !cc1) c1;
...
(* assert (rep cc2 c2 = rep cc2 c2'); (* TODO: OK to *make* them equal? *)  *)
    ...
    *)
    ...
    let no_live ts nr =
        List.for_all
          (fun x -> get_unifiable ts x <> Standard)
...
(* This test is hard to automatically check.  Approximate by checking a size *)
       ...
       let _, ccts = merge_cc subst cc ts in
