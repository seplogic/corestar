...
let rec term_to_smt_term t = 
  ...
  in Expression.cases if_var if_app t 
...
let push i = (* ( push <numeral> ) *)
  let x = sprintf "( push %s )" (string_of_int i) in
  smt_command x    

let pop i =  (* ( pop <numeral> ) *)
  let x = sprintf "( pop %s )" (string_of_int i) in
  smt_command x    
